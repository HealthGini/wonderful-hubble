...
class TestRegressionCoverage(GoodDeedsTestCase):
    ...
    def test_admin_cannot_promote_or_demote_self_or_site_admin_in_space_roster(self):
        """
        Verifies that:
        1. Admins in a Space roster display 'Space Admin' (not 'Site Admin') and return is_site_admin=1 / group.is_admin=True.
        2. renderGroupRoster in static/app.js guards role/kick buttons with `canManageRoles && !isSelf && !isMemberSiteAdmin`
           so no admin sees 'Promote to Admin' or 'Demote from Admin' on themselves or on a Space Admin.
        3. Sent Space Invitations Roster is full-width (`col-span-full`), non-truncated, and clickable (`openSpaceInvitationModal` / `modal-space-invitation-detail`).
        4. Backend /api/groups/<id>/members/role rejects self-role changes with 400.
        """
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        with open(os.path.join(base_dir, "static", "index.html"), "r", encoding="utf-8") as f:
            html = f.read()
        ...
        self.assertIn("Space Admin", js)
        self.assertNotIn(">Site Admin<", js)
        self.assertIn('id="modal-space-invitation-detail"', html)
        self.assertIn("openSpaceInvitationModal", js)
        self.assertIn("🔍 View Invitation Details", js)

        token_maya = self.get_token("maya@gooddeeds.space")
        headers_maya = {"Authorization": f"Bearer {token_maya}"}

        # Check Group 1 roster and invitations for Maya_Lin (User 1)
